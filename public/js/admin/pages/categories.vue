   	...
   	<div class="content">
		<div class="container-fluid">
			...
			<div class="_1adminOverveiw_table_recent _box_shadow _border_radious _mar_b30 _p20">
				<p class="_title0">Categories <Button size="small" @click="modal.catAddModal = true"> <Icon type="md-add"/> Add</Button></p>
					...
					<table class="table table-striped table-hover">
						...
						<tbody>
							<tr v-for="(cat, i) in allCats" :key='i' v-if="allCats.length">
								...
								<td>
									<Tooltip content="Edit category" placement="top-start">
							            <Button type="primary" icon='ios-create-outline' size='small' @click="showEditModal(cat,i)">
											Edit
										</Button>
							        </Tooltip>
									<Tooltip content="Delete category" placement="top-start">
...
<script type="text/javascript">
	...
	export default {
		...
		methods: {
            ...
            async onDeleteCat(){
            	// this.modal.isAdding = true;
            	// const res = await this.__callApi('post', '/app/delete_cat', this.deleteModal);
            	// if(res.status == 200){
            	// 	this.sc("Category deleted successfully");
            		
            	// 	if(this.helpers.deleteImageWithCat && this.deleteModal.iconImage != null){
	            // 		var filePath = this.deleteModal.iconImage;
	            // 		let fileName = filePath.split('/').slice(-1)[0].replace(/^\/|\/$/g, '');
            	// 		const deleteRes = await this.__callApi('post', '/app/delete_upload_file', {iconImage: fileName});
